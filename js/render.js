...
const Renderer = (() => {
  ...
  function renderHero() {
    ...
    const contactLinks = `
      ...
      <a href="https://mail.google.com/mail/?view=cm&fs=1&to=${(d.contact || {}).email || 'surajkumar11292@gmail.com'}" target="_blank" rel="noopener noreferrer" class="contact-link">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
          <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/>
          <polyline points="22,6 12,13 2,6"/>
        </svg>
        ${(d.contact || {}).email || ''}
        <button class="copy-btn" data-copy-email aria-label="Copy email" onclick="event.preventDefault(); event.stopPropagation(); navigator.clipboard.writeText('${(d.contact || {}).email || ''}'); if(typeof UI!=='undefined'&&UI.Toast) UI.Toast.show('Email copied to clipboard!', '✓');">Copy</button>
      </a>
    ...
    const links = [
      {
        ...
        label: d.email,
        href: `https://mail.google.com/mail/?view=cm&fs=1&to=${d.email}`,
        external: true,
      },
      {
