  ...
  function renderHero() {
    ...
    const contactLinks = `
      ...
      <a href="mailto:${(d.contact || {}).email || ''}" class="contact-link"
         onclick="event.preventDefault(); window.open('https://mail.google.com/mail/?view=cm&fs=1&to=${(d.contact || {}).email || ''}', '_blank'); navigator.clipboard.writeText('${(d.contact || {}).email || ''}'); if(typeof UI!=='undefined'&&UI.Toast) UI.Toast.show('Opening Gmail & copied email!', '✉');">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
          <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/>
          <polyline points="22,6 12,13 2,6"/>
        </svg>
        ${(d.contact || {}).email || ''}
        <button class="copy-btn" data-copy-email aria-label="Copy email" onclick="event.stopPropagation(); navigator.clipboard.writeText('${(d.contact || {}).email || ''}'); if(typeof UI!=='undefined'&&UI.Toast) UI.Toast.show('Email copied to clipboard!', '✓');">Copy</button>
      </a>
        ...
        if (link.isEmail) {
          return `
            <a href="${link.href}"
               ...
               onclick="event.preventDefault(); window.open('https://mail.google.com/mail/?view=cm&fs=1&to=${d.email}', '_blank'); navigator.clipboard.writeText('${d.email}'); if(typeof UI!=='undefined'&&UI.Toast) UI.Toast.show('Opening Gmail & copied email to clipboard!', '✉');">
              ${link.icon}
              ${link.label}
            </a>`;
        }
